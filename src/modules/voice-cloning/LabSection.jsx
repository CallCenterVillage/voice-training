...
const LabSection = () => {
  ...
  const exercises = [
    ...
    { title: "4: Detection", desc: "Now put on your defender hat — use Resemblyzer, librosa, and SoX to analyze your clone from multiple angles. Which tools catch the fake? Which modified voices fool the similarity checks?", cmd: `# === Speaker Similarity (Resemblyzer) ===\n# Compare your original voice against the AI clone\nresemblyzer my_voice.wav clone.wav\n\n# Try it against your modified voices too\nresemblyzer my_voice.wav v1_higher.wav\nresemblyzer my_voice.wav v2_deeper.wav\nresemblyzer my_voice.wav v3_phone.wav\nresemblyzer my_voice.wav v4_robot.wav\n\n# === MFCC Similarity (librosa) ===\n# A different similarity approach — uses spectral features instead of embeddings\nvoice-similarity my_voice.wav clone.wav\nvoice-similarity my_voice.wav v1_higher.wav\nvoice-similarity my_voice.wav v2_deeper.wav\n\n# === Artifact Scan (librosa) ===\n# Scan for spectral anomalies that indicate AI generation\nvoice-artifact-scan clone.wav\nvoice-artifact-scan my_voice.wav      # Compare — your real voice should have no flags\n\n# === Side-by-Side Spectrograms (librosa) ===\n# Generate a visual comparison of your real voice vs the clone\nvoice-spectrogram my_voice.wav clone.wav detection_comparison.png\nimgcat detection_comparison.png\n\n# === Batch Analysis (librosa) ===\n# Scan all your recordings at once and get a CSV report\nvoice-batch-analyze . voice_analysis_report.csv\ncat voice_analysis_report.csv\n\n# === Visual Detection (SoX Spectrograms) ===\n# Generate spectrograms for your original and cloned voices\nsox my_voice.wav -n spectrogram -o spec_original.png\nsox clone.wav -n spectrogram -o spec_clone.png\nimgcat spec_original.png\nimgcat spec_clone.png\n\n# What to look for:\n# - Metallic shimmer: unnatural energy above 8kHz (vocoder artifacts)\n# - Missing room tone: AI audio often has unnaturally clean silence\n# - Flat pitch contour: AI may lack natural pitch variation\n# - Repeating patterns: look for too-perfect harmonic structure\n\n# === Frequency Statistics ===\n# Compare the stats between original and clone\necho "=== Original ===" && sox my_voice.wav -n stat 2>&1\necho "=== Clone ===" && sox clone.wav -n stat 2>&1` },
  ];
  return (<div>
