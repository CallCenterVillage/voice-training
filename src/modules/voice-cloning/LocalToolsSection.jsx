...
const LocalToolsSection = () => (<div>
  ...
  <ToolComparison tools={[
    ...
    { name: "Qwen3-TTS", desc: "Alibaba's LLM-based TTS (Apache-2.0, Jan 2026). Clones a voice from roughly 3 seconds of reference audio, or invents a brand-new voice from a written description. 10 languages, 9 preset speakers.", pros: ["3-second cloning", "Voice design", "Apache-2.0"], cons: ["GPU expected", "No synthesis CLI"], install: `# Preinstalled on Call Center Village laptops\n# Wrapper at /usr/local/bin/qwen3-tts → qwen-tts-demo (Gradio web UI)\n\n# Upstream ships only ONE console script: the web demo.\n# If the service is running, just open it in your browser:\n#   http://localhost:8000\n# Otherwise launch it yourself:\nqwen3-tts Qwen/Qwen3-TTS-12Hz-1.7B-CustomVoice --ip 0.0.0.0 --port 8000\n\n# Smaller model, and skip flash-attn if it isn't installed:\nqwen3-tts Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice --no-flash-attn --port 8000\n\n# Cloning has no CLI — it's Python-only. Note ref_text must be an\n# exact transcript of ref_audio, and ~3 seconds is genuinely enough.\ncd ~/callcentervillage/voice-cloning\n/opt/qwen3-tts/.venv/bin/python3 << 'EOF'\nfrom qwen_tts import Qwen3TTSModel\nimport soundfile as sf\n\nmodel = Qwen3TTSModel.from_pretrained(\n    "Qwen/Qwen3-TTS-12Hz-1.7B-Base", device_map="cuda:0")\n\nwavs, sr = model.generate_voice_clone(\n    text="This is an AI clone of my voice. Pretty wild, right?",\n    language="English",\n    ref_audio="my_voice.wav",\n    ref_text="Exact transcript of what is said in my_voice.wav")\n\nsf.write("clone.wav", wavs[0], sr)\nEOF\nplay clone.wav` },
  ]} />
    <SectionDivider />
